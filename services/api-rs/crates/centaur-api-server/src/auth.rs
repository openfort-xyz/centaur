...
const INGRESS_SPECS: &[IngressSpec] = &[
    ...
    // Google Chat keys always include `spaces:`. Bare `chat:C…` keys belong
    // to the legacy Slack-compatible adapter and stay out of this caller's scope.
    IngressSpec {
        env_var: "GOOGLECHATBOT_API_KEY",
        identity: "googlechatbot",
        platform_prefixes: &["chat:spaces:"],
        workflow_events: true,
    },
];
