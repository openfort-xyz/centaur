...
| `googlechatbot.*` | Google Chat ingress, state, delegated operations, and rendering. See [Google Chat](/reference/google-chat). |
...
| `GOOGLECHATBOT_CODEX_NANOCODEX_ROLLOUT_PERCENT` | `googlechatbot.codexNanocodexRolloutPercent`. | Google Chat counterpart to the Slack rollout. It uses the same deterministic thread-key assignment, metadata, model-override bypass, and `0` default. |
