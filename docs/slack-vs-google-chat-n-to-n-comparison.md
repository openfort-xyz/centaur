...
Google Chat parity is **not yet exhaustively verified**. The deployed release
passes real signed Workspace Add-on ingress, same-message response rendering,
inbound image materialization, DWD, file, reaction, Drive, and ETL checks.
The final DM history/update credential-selection fixes are deployed and pass the
exact eleven-destination matrix. The remaining gates are listed below. Release truth is
...
6. Google Chat's selected Workspace Add-on mode, DWD, upload/download, reaction,
   Drive, mutation, and shared/DM ETL paths have production Workspace evidence.
   A real named-space UI mention completed through the signed endpoint; the
   other ingress modes and Add-on action/command/form negatives remain.
...
| Conversation grants | Exact Slack conversation IDs with independent upload/download/history flags, on principals or roles. | Exact `spaces/<id>` with independent send/update/delete/upload/download/history/members/reactions flags, on principals or roles. The inventory is exactly five named channels and six bot DM/group-DM destinations; server-selected delegated readers remain internal claims rather than agent-chosen identities. | Google has broader operation coverage. Bounded reads passed 77/77 across all eleven; explicit history/member/reaction checks passed 33/33 and an unauthorized destination was denied. |
...
| Grant merge/cache | Direct and role grants contribute effective claims and invalidate principal configuration. | Same OR merge and cache invalidation. Targets are immutable; duplicate/empty rows rejected. | Parity. Console permission CRUD still needs the separate browser interaction gate. |
...
| Delegated credentials | Optional Slack user/search/OAuth tokens. | Fixed DWD subjects cover upload (`chat.messages.create`), reactions (`chat.messages.reactions.readonly`), Drive (`drive.readonly`), and exact per-space reads/mutations selected by the server; the validated target is the DM-setup subject (`chat.spaces.create`). Agent input cannot choose the delegated reader. | Platform-specific; delegated subjects must be impersonable same-domain Workspace users and authorized for the destination. |
| Missing advanced credential | Operation fails or returns missing-scope error. | Upload, DM history/mutation, and reactions fail closed without the exact server-selected subject; Drive content degrades to metadata. | Parity in least privilege. |
...
| Mention detection | Exact configured Slack bot ID in text/rich content. | Exact Chat annotation/resource identity; slash-command annotation is addressed. Ordinary `@alice`, another bot, and self produce no session call. | A real exact named-space mention passed; the DM/slash/self/other-bot live matrix remains. |
...
| Members | Proxied/direct channel members and email-oriented helpers. | Paginated authorized space memberships. | Core parity; membership reads passed in all eleven app-visible destinations. Slack has richer workspace-profile resolution. |
...
| Update | Renderer updates its own Slack response; no generic agent CLI. | Scoped `update-message`; app auth updates app-authored messages, while an exact server-selected delegated reader can update a verified delegated Centaur message. Client-assigned IDs never grant ownership. | Google-native extension, not a Slack parity requirement. Fresh named and DM bot updates plus delegated uploaded-message update passed. |
| Delete | No generic agent command. | Scoped `delete-message` after app or exact delegated ownership verification. | Google-native extension, not a Slack parity requirement. Fresh app and delegated delete/cleanup passed. |
...
| Capability | Slack | Google Chat | Current difference |
| --- | --- | --- | --- |
| Inbound file hydration | Slack files downloaded and typed through the shared attachment input. | `UPLOADED_CONTENT` downloaded through Chat media API and passed through the same materialized attachment input; Drive files use a separate DWD Drive reader. | Parity; a real image was materialized and used visually. Deterministic upload/metadata/download returned exact 35 bytes with SHA-256 `6d1eb9507fe0d0bf6dae8025800c1355279e468998498cb16fc313daee4d11fc`; a live 75,979-byte XLSX Drive export also passed. |
...
| File download | Raw direct/proxy path; host validation; CLI ceiling. | Raw proxy stream, 100 MiB server ceiling, HTML rejection, sanitized filename, 10 MiB CLI default, no overwrite. | Parity for the exact small-file path; 35 bytes and SHA-256 matched live. Large declared/chunked boundaries remain pending. |
...
The agent-visible scope is deliberately the eleven authorized destinations the
Chat app can operate in: five named channels and six bot DM/group-DM spaces. Additional
personal or meeting spaces visible only through a delegated user are not agent
destinations: the proxy exposes only exact granted space IDs, and any delegated
reader is selected and signed by the server rather than supplied by the agent.
...
| CI | Typecheck and repeated Slack suites. | Typecheck/full suite, signed fixture smoke, CLI, Rust, Rails, workflow, Helm/schema, image-build, Argo, and production verification gates. | Full CI and CodeQL passed. Build run `32029109132` attempt 2 succeeded after a transient Slack-image Docker Hub 502; five images passed first try and the rerun passed. Argo was `Synced`, `Healthy`, and `Succeeded` with exact `ad844b…` images for api-rs, googlechatbot, Console, Console worker, and api-proxy. Independent security review also passed. |
...
1. File boundaries at 25 MiB, 25 MiB + 1, 100 MiB, and 100 MiB + 1 with staged
   reconstruction and SHA-256 verification. The exact 35-byte path has passed.
2. Actual narrow/wide keyboard-only Console grant workflows with screenshots.
3. Explicit live ETL continuation, cross-owner negative RLS, rate-limit/
   permission recovery, and retention scenarios.
4. Live shared-space and delegated-DM deletion convergence using
   `showDeleted=true`, plus reaction-removal convergence and derived-row cleanup.
5. The remaining Google-native Drive export types and above-10-MB error path;
   the live XLSX below-limit path passed.
6. Project-number and endpoint-URL ingress, DM/slash/self/bot cases, Add-ons
   action/command/form payloads, and their live negative cases.
7. The complete TEST-031 matrix passing on the same immutable release evidence
   as every TEST-001 through TEST-030 row.

## Bottom line

The deployed release implements most required Google Chat outcomes without
copying Slack-only abstractions. A real signed named-space UI turn,
same-message/no-tombstone rendering, exact app identity, all-eleven authorized
reads, app/delegated mutations, exact small-file integrity, unauthorized-space
denial, DWD, reactions, Drive XLSX export, and owner-scoped ETL are live-proven.
All final images, CI, CodeQL, Argo health, and independent security review
passed. The remaining work is the broader matrix above and the
same-immutable-evidence gate. Slack remains the fully production-proven
