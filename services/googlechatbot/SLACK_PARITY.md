...
| 3.2 | #875 console Slack thread visibility for Slack SSO identities | Chat threads were invisible in the threads view. Added `googlechat_thread_owner_sql`: googlechatbot now records the requester's workspace email (`user_email`, from `sender.email`/envelope `user.email`) in message metadata, and in session metadata only when the request was signature-verified and the sender's domain is allowlisted (`resolveIdentityEmission`) — centaur labels DM principals off the session keys, so an unverified envelope must not supply them; console matches it against the signed-in (Google SSO) user's email directly — simpler than Slack's identity mapping because console logins ARE Google identities. | ✅ |
