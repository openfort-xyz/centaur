...
| TASK-015 | TEST-015 | Provisional | 2026-08-17 | `ad844b2565bdae8d830663bac5437193ecdf76dd` | production Workspace + deterministic E2E/PostgreSQL | Inspect the production reply thread; run deterministic crash/recovery tests. | The inspected production thread contained one human root and one bot reply; the reply's final content patched its `client-centaur-ack` acknowledgement and left no tombstone. Automated crash/recovery passes; a live restart drill remains. | `services/googlechatbot/src/index.e2e.test.ts`, `services/googlechatbot/src/state.ts`; sanitized production observation |
...
| TASK-018 | TEST-018 | Provisional | 2026-08-17 | `ad844b2565bdae8d830663bac5437193ecdf76dd` | production Workspace + release CI | Inspect acknowledgement PATCH, thread placement, and Console card; run focused client tests. | Live named-space threading, acknowledgement PATCH, Console card, `client-centaur-ack`, and no-tombstone lifecycle passed. The focused client suite passed 52 tests / 149 expectations; message-size, quota, and multi-replica Workspace boundaries remain. | `services/googlechatbot/src/chat/client.test.ts`, `services/googlechatbot/src/index.test.ts`, `services/googlechatbot/src/renderer.test.ts`; sanitized production observation |
| TASK-019 | TEST-019 | Provisional | 2026-08-17 | `ad844b2565bdae8d830663bac5437193ecdf76dd` | production scoped sandbox + release CI | Bounded authorized read matrix and ungranted-destination probe. | The bounded read matrix passed 77/77 across exactly eleven destinations; explicit history, membership, and reaction checks passed 33/33. Unauthorized-space access was denied. | `tools/comms/google_chat/client.py`, `tools/comms/google_chat/cli.py`, `tools/comms/google_chat/tests/`; sanitized production observation |
...
| TASK-022 | TEST-022 | Provisional | 2026-08-17 | `ad844b2565bdae8d830663bac5437193ecdf76dd` | production Workspace + deterministic shared fixtures | Inbound image turn; deterministic file upload, metadata lookup, and download. | A real inbound image was materialized and used by the model. A deterministic uploaded text file returned exact metadata and 35 download bytes with SHA-256 `6d1eb9507fe0d0bf6dae8025800c1355279e468998498cb16fc313daee4d11fc`; large boundaries and remaining live card shapes remain. | `fixtures/google_chat_card_text.json`, `services/googlechatbot/src/chat/card-text.test.ts`, `services/googlechatbot/src/chat/normalize.test.ts`, `workflows/tests/test_google_chat_etl.py`; sanitized production observation |
