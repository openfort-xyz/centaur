...
function claims(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return { iss: GOOGLE_CHAT_SA_ISSUER, aud: AUD, iat: NOW, exp: NOW + 300, ...overrides }
}
...
describe('verifyGoogleSignedJwt', () => {
  ...
  test('rejects a tampered payload', async () => {
    ...
    const out = await verifyGoogleSignedJwt({ token: forged, audiences: [AUD], allowedIssuers: [GOOGLE_CHAT_SA_ISSUER], nowSeconds: NOW, resolveKey })
    expect(out).toEqual({ ok: false, reason: 'bad_signature' })
  })
  ...
  test('rejects a structurally malformed token', async () => {
    const out = await verifyGoogleSignedJwt({ token: 'not-a-jwt', audiences: [AUD], allowedIssuers: [GOOGLE_CHAT_SA_ISSUER], nowSeconds: NOW, resolveKey })
...
describe('googleRequestKeyResolver (JWK set caching)', () => {
  test('imports the JWK set, caches it, and resolves by kid', async () => {
    ...
    const resolver = googleRequestKeyResolver({ fetchImpl, nowMs: () => 0 })
    const first = await resolver(KID, GOOGLE_CHAT_SA_ISSUER)
    const second = await resolver(KID, GOOGLE_CHAT_SA_ISSUER)
    expect(first).not.toBeNull()
    expect(second).not.toBeNull()
    expect(fetches).toBe(1) // second call served from cache
    expect(await resolver('unknown-kid', GOOGLE_CHAT_SA_ISSUER)).toBeNull()

    // A real signed token verifies against the resolver-imported public key.
    const token = await signJwt({ privateKey: pair.privateKey, kid: KID, claims: claims() })
    const out = await verifyGoogleSignedJwt({ token, audiences: [AUD], allowedIssuers: [GOOGLE_CHAT_SA_ISSUER], nowSeconds: NOW, resolveKey: resolver })
    expect(out.ok).toBe(true)
  })
})
...
describe('googleRequestKeyResolver (both token models)', () => {
  ...
  test('accepts the SA project-number token (iss=chat@system) via the SA set', async () => {
    ...
    const token = await signJwt({
      ...
      kid: SA_KID,
      claims: { iss: GOOGLE_CHAT_SA_ISSUER, aud: '734836800829', iat: NOW, exp: NOW + 300 }
    })
    const out = await verifyGoogleSignedJwt({
      token,
