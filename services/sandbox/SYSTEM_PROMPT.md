...
|Tasks may omit a schedule for manual-only use. Recurring tasks use five-field cron expressions in Pacific Time. Use `dm` as the delivery channel for the user's direct message on the deployment's chat platform (their linked Slack DM, or their Google Chat DM otherwise); or use a Slack channel ID or Google Chat space name (`spaces/<id>`) allowed by their current delivery permissions.
