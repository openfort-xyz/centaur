...
| Google Chat space, including a 1:1 DM space | `gchat-space-<space-id>` | Every thread and message in that space |
| Google Chat requester (per turn) | `gchat-user-<email>-<sha256[:12]>` | That person, in any space they write from |
...
Granting a Slack, Discord, or Teams channel principal, or a Google Chat space
principal, gives the permission to every Centaur session that resolves to that
principal. Use a user principal for personal credentials and a channel
principal only for access that all participants should share.
:::

Google Chat sessions always key on the space, so a 1:1 DM space principal
identifies one person only by accident of membership. Personal grants belong on
the `gchat-user-<email>-<sha256[:12]>` principal instead (email lowercased and
slugged, then the first 12 hex of its sha256 so slug-alike addresses never
share a principal): Centaur binds it as the requester
of every turn that person starts, in a DM or a group space, and its direct
static-secret grants join that turn's proxy configuration without the space
principal ever holding them. The email must have been verified from the
Workspace Add-on `userIdToken` and pass the domain allowlist; unverified
senders bind no requester.
