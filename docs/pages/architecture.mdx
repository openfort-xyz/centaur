...
Centaur accepts Slack, Google Chat, and API requests, stores each turn, assigns an isolated
...
|-------|----------------|-----------------|
| Ingress | Accept user and client input. | Slack and Google Chat webhooks, external API clients. |
...
`googlechatbot` verifies Google-signed requests, persists accepted work in
Postgres, and calls the same session API with `GOOGLECHATBOT_API_KEY`. It edits
a thinking acknowledgement with the final answer and retains unfinished
render obligations for recovery. Agent tools call api-rs with scoped Console
JWTs; api-rs authorizes the exact operation and space before calling the bot's
private Google API routes. See [Google Chat](/reference/google-chat) for setup
and the remaining verification gates.
